...
def sampleIMAGES():
    ...
    for i in range(10000):
        randomnum = np.random.randint(0,10) #[0,10)
        image = IMAGES[:,:,randomnum]
        width, length = image.shape
#        print(width, length)
        w = l = 8
        x = np.random.randint(0, width+1-w)
        y = np.random.randint(0, length+1-l)
        cropImg = image[(x):(x + w), (y):(y + l)]
        crop_reshape = cropImg.reshape((1, 64)) #reshape to 64-dimensional vector
#        print(crop_reshape)
        patches[i] = crop_reshape
#        print(patches)
...
def sparseAutoencoderCost(theta,visibleSize,hiddenSize,decayWeight,sparsityParam,beta,data):
    ...
    #output layer
    delta_out = (a3 - y) * a3 * (1.0 - a3)
    #hidden layer
    sparsity_avg = np.sum(a2, axis=0) / np.float32(data.shape[0])
    sparsity_term = -sparsityParam / sparsity_avg + (1.0 - sparsityParam) / (1.0 - sparsity_avg)
    delta_hid = (np.dot(delta_out, W2.T) + beta * sparsity_term) * a2 * (1.0 - a2)
    
    #Backpropagation
    W1grad = np.dot(a1.T, delta_hid) / np.float32(data.shape[0]) + decayWeight * W1
    W2grad = np.dot(a2.T, delta_out) / np.float32(data.shape[0]) + decayWeight * W2
    b1grad = np.sum(delta_hid, axis=0) / np.float32(data.shape[0])
    b2grad = np.sum(delta_out, axis=0) / np.float32(data.shape[0])

    sum_of_squares_error = 0.5 * np.dot((a3 - y).ravel(), (a3 - y).ravel()) / np.float32(data.shape[0])
    weight_decay = 0.5 * decayWeight * (np.dot(W1.ravel(), W1.ravel()) + np.dot(W2.ravel(), W2.ravel()))
    KL = beta * (np.sum(sparsityParam * np.log(sparsityParam / sparsity_avg) + (1.0 - sparsityParam) * np.log((1.0 - sparsityParam) / (1.0 - sparsity_avg))))
    cost = sum_of_squares_error + weight_decay + KL
...
def computeNumericalGradient(J,theta):
    ...
    for i in range(len(theta)):
        theta_tem = theta[i]
        theta[i] = theta_tem + epsilon
        theta_p = J(theta)
#        print(theta_p)
        theta[i] = theta_tem - epsilon
        theta_m = J(theta)
#        print(theta_m)
        numgrad[i] = 0.5 * (theta_p - theta_m) / epsilon
        theta[i] = theta_tem
...
def displayNetwork(A,optNormEach=False,optNormAll=True,numColumns=None,imageWidth=None,cmapName='gray',
    ...
    if graphicsLibrary=='PIL':
        ...
        if verbose:
            im.save(saveName) #im.save before im.show, otherwise save empty image
            im.show()
#        if saveName=='':
#            im.save(saveName)
